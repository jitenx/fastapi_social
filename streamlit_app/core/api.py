...
def post(endpoint, payload=None):
    response = requests.post(f"{API_BASE_URL}{endpoint}", json=payload, headers=auth_header())
    return _handle_response(response)

def put(endpoint, payload=None):
    response = requests.put(f"{API_BASE_URL}{endpoint}", json=payload, headers=auth_header())
    return _handle_response(response)
